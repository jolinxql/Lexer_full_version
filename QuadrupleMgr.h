...
public:
	...
	Quadruple* NewQuad(int aOpr,Item* aSrc1,Item* aSrc,Item* aObj);
	Item* NewItem(int atype, SymbolTable* aParentTable);
	Item* NewItem(int atype, SymbolTable* aParentTable,int aProcNumber);
	void NewItemForInVar(Item* i);	//not variables
	void NewItemForInVarArray(Item*i,int size);	//not variables
	Item* NewCItem(int atype,string avalue);//Jolin:char?
	Item* NewLItem();
	Item* NewSItem(string avalue);//Jolin:String?
