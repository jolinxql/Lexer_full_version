...
class Parser{
public:
	FILE *in;	// the Parser has its own input file
	Lexer lex;	// the Parser has its own lexer
	vector<Token> reserveWord;	// the Parser has its own reserveWord list
	fpos_t pos,dpos,tPos;	// the position pointers
	Token tempToken;	// the current Token
	int tempLineNo;
	int tempSerialNo;
	int lineNo;
	int serialNo;
...
//	Parser();
	...
	void reserve(Token w);	//to reserve the new Token in the reserve Wordlist
	int getPos(FILE* fp,__int64* pose);
	int setPos(FILE* fp,__int64* pose);
	bool checkKeyToken(string key);
	bool checkKeyTokenR(string key);	//Jolin:???
	bool checkId();
	bool checkIdR();		//Jolin:???

	void Chr(int& s);
	void Program();		//program declaration
//Definition & Declaration
	void ConstDec();	// const declaration
	void ConstDef();	// const definiton
	void VarDec();		// variable declaration
	void VarDef();		// variable definition
	void ValuedFuncDef();//function definition
	void VoidFuncDef();	// procedure definition
	void MainDef();
//mixed staff
	void Parameters(string n);
	void CompStat();	// compound statement
	void StatList();
	void ParaTable(string n);
	void Stat();
	void Expr(Item*& i); // Syhthesized attribution i
	void Term(Item*& i); // Syhthesized attribution i
	void Factor(Item*& i);//Syhthesized attribution i
	void Interger(int& s);
	void checkNumR();
//Statement:
	void AssignStat();
