...
QuadrupleMgr::QuadruppleMgr()
{
	...
	SItemNumber = 0;
	RItemNumber = 4;
	RItemList[0] = new Item("@EBX", REG, _INT, "ebx", 0, NULL);//16bit General Reg
	RItemList[1] = new Item("@ESI", REG, _INT, "esi", 0, NULL);//16bit Source Index
	RItemList[2] = new Item("@EDI", REG, _INT, "edi", 0, NULL);//16bit Destination index
	RItemList[3] = new Item("@EBP", REG, _INT, "ebp", 0, NULL);//16bit Base Pointer
